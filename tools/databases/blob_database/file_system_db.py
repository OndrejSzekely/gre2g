...
class FileSystemDB(BaseBlobDB):
    ...
    def delete_level(self, level_path: List[str]) -> None:
        ...
        try:
            ...
            raise OSError(f"Level `{level_path}` can not be deleted.") from os_error
    ...
    def add_file(self, level_path: List[str], file: bytes, file_name_with_ext: str, use_hash: bool = True) -> None:
        ...
        try:
            ...
            raise OSError(f"File `{file_name_with_ext}` can not be added.") from os_error
    ...
    def delete_file(self, file_path: List[str]) -> None:
        ...
        try:
            ...
            raise OSError(f"File `{file_path}` can not be deleted.") from os_error
    ...
    def _load_level_mapping_table(self, level_path: List[str]) -> pd.DataFrame:
        ...
        return PDDFIOHandler.load_df(self._get_mapping_table_abs_path(self._get_fs_path(level_path)),
                                     self.pd_df_fs_format)
